...
- `ad-slot.html`에 쿠팡 파트너스 배너/다이나믹 배너 스크립트를 붙여넣으면 홈·전체 코드 페이지 하단에 광고가 표시된다.
- 광고가 있을 때만 법정 고지문구("본 사이트는 쿠팡 파트너스 활동의 일환으로…")가 자동 부착된다. 비어 있으면 슬롯 자체가 렌더되지 않는다.
- 상품검색 API는 시간당 10회 제한이라 실시간 조회는 부적합 → 다이나믹 배너(클라이언트 스크립트) 방식 사용.
- 트래킹코드는 클라이언트에 노출되는 값이라 파일에 두어도 무방(비밀키 아님).
